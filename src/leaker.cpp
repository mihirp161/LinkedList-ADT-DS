...
void _Leaker_Dump(void)
{
	...
	else
	{
		...
		unsigned int i;
		fprintf(stdout, "%u allocations (%u bytes) in table of %u rows.\n",
	...
	if (_leaker.mismatches)
		fprintf(stdout, "Mismatches: %u allocation/deallocations don't match!\n",
			_leaker.mismatches);
	if (_leaker.overflows)
		fprintf(stdout, "Overflows: %u allocations overflowed (wrote off end)!\n",
			_leaker.overflows);
	if (_leaker.bad_frees)
		fprintf(stdout, "Bad deallocs: %u attempts made to deallocate unallocated pointers!\n",
			_leaker.bad_frees);
	...
	const char* file, const char* func, size_t line)
{
	...
	{
		fprintf(stdout, "%s:%s():%u fatal error: address %p already in use!\n",
			file, func, line, addr);
		exit(2);
	}
	...
	const char* func, size_t line)
{
	...
	{
		fprintf(stdout, "\nLEAKER: %s:%s():%u %s error: pointer was not allocated!\n\n",
	...
	*mover = temp->next;
	...
	{
		fprintf(stdout, "\nLEAKER: %s:%s():%u checking error: wrote off end of memory allocated at %s:%s():%u.\n\n",
			file, func, line, temp->file, temp->func, temp->line);
		_leaker.overflows++;
	}
	if (!_Leaker_Check_Dealloc(temp->alloc, dealloc)) /* wrong dealloc function */
	{
		fprintf(stdout, "\nLEAKER: %s:%s():%u mismatch error: memory allocated at %s:%s():%u with %s, deallocated with %s.\n\n",
			file, func, line, temp->file, temp->func, temp->line,
			temp->alloc, dealloc);
		_leaker.mismatches++;
	}
...
static void _Leaker_Report(void)
{
	...
	if (_leaker.count) /* print out list of leaks, clean up */
	{
		...
		fprintf(stdout, "Leaks found: %u allocations (%u bytes).\n",
			_leaker.count, _leaker.bytes - _leaker.count * GUARD_SIZE);
...
static void _Leaker_Print_Entry(_LEAK_T* entry)
{
	fprintf(stdout, "%s:%s():%u memory leak: memory was not deallocated.\n",
		entry->file, entry->func, entry->line);
	if (!_Leaker_Check_Guard(entry->addr, entry->size))
	{
		fprintf(stdout, "%s:%s():%u checking error: wrote off end of allocation.\n",
			entry->file, entry->func, entry->line);
		_leaker.overflows++;
	}
}
...
static void _Leaker_Dump_Entry(_LEAK_T* entry)
{
	fprintf(stdout, "%s:%s():%u address: %p bytes: %u",
